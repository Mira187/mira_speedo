...
$(document).ready(function(){
    ...
    window.addEventListener('message', function( event ) {   
        ...
        if (event.data.vehicleon == false) {
            document.getElementById("icon1").innerHTML = '<img id="Ebene_11" src="item/off.png">'
        };
        if (event.data.vehicleon == true) {
            document.getElementById("icon1").innerHTML = '<img id="Ebene_11" src="item/on.png">'
        };
        
        if (event.data.locked == false) {
            document.getElementById("icon2").innerHTML = '<img id="unlocked" src="item/unlocked.png">'
        };
        if (event.data.locked == true) {
            document.getElementById("icon2").innerHTML = '<img id="unlocked" src="item/locked.png">'
        };
    });
});
